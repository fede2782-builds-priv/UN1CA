...
cp -a --preserve=all "$FW_DIR/${MODEL}_${REGION}/product/priv-app/HotwordEnrollmentOKGoogleEx4RISCV" "$WORK_DIR/product/priv-app"
rm -rf "$WORK_DIR/product/priv-app/HotwordEnrollmentXGoogleEx4CORTEXM55"
cp -a --preserve=all "$FW_DIR/${MODEL}_${REGION}/product/priv-app/HotwordEnrollmentXGoogleEx4RISCV" "$WORK_DIR/product/priv-app"
...
echo "Replacing with stock HWC"
REMOVE_FROM_WORK_DIR "$WORK_DIR/system/system/bin/surfaceflinger"
REMOVE_FROM_WORK_DIR "$WORK_DIR/system/system/lib/libgui.so"
REMOVE_FROM_WORK_DIR "$WORK_DIR/system/system/lib/libui.so"
REMOVE_FROM_WORK_DIR "$WORK_DIR/system/system/lib64/libgui.so"
REMOVE_FROM_WORK_DIR "$WORK_DIR/system/system/lib64/libui.so"
