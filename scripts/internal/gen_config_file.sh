...
GET_OFFICIAL_STATUS()
{
    local USES_REAL_CERT=false

    if [ -f "$SRC_DIR/real/security/real_platform.pk8" ]; then
        openssl ec -pubout -in "$SRC_DIR/real/security/real_platform.pk8" -out "$OUT_DIR/real_platform.pub" &> /dev/null
        if cmp -s "$SRC_DIR/real/security/real_platform.pub" "$OUT_DIR/real_platform.pub"; then
            USES_REAL_CERT=true
        fi
        rm -f "$OUT_DIR/real_platform.pub"
    fi
...
GEN_CONFIG_FILE()
{
    ...
    {
        echo "# Automatically generated by real/scripts/internal/gen_config_file.sh"
